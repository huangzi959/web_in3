...
<body>
         ...
         <div class="w-set-con">
          ...
          <div class="w-set-nr">
              <div class="water-mark ml-10" style="padding-top:20px">
                ...
                {if empty($config['cfg_cms_51'])}
                  <div class="rowElem" style="margin-top: 30px;">
                      <label>系统版本选择：</label>
                      <input type="radio"  name="cfg_sys_version" value="4.1" {if $config['cfg_sys_version']=='4.1'}checked{/if}>
                      <label>4.1版本</label>
                      <input type="radio"  name="cfg_sys_version" value="5.1" {if $config['cfg_sys_version']=='5.1'}checked{/if}>
                      <label>5.1版本</label>
                  </div>
                {/if}
    ...
    * 初始化版本参数
    * */
   /* function init_version_param()
    {
        var url = SITEURL+'config/ajax_init_version_param';
