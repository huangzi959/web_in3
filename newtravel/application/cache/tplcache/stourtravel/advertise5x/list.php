...
<head>
    <meta charset="utf-8">
    <title>广告管理-思途CMS<?php echo $coreVersion;?></title>
        ...
        <td valign="top" class="content-rt-td" style="overflow:hidden">
            ...
            <div class="filter">
                ...
                <select name="adtype" id="adtype" class="bd_style wid_100" onchange="goSearch(this.value,'adtype')">
                    ...
                    <?php $n=1; if(is_array($position)) { foreach($position as $v) { ?>
                    <optgroup label="<?php echo $v['kindname'];?>" style="background: #f4f4f4;"></optgroup>
                    <?php $n=1; if(is_array($v['sub'])) { foreach($v['sub'] as $r) { ?>
                    <option value="<?php echo $r['pagename'];?>">&nbsp;&nbsp;<?php echo $r['kindname'];?></option>
                    <?php $n++;}unset($n); } ?>
                    <?php $n++;}unset($n); } ?>
                </select>
...
Ext.onReady(
    function () {
        ...
        $("#addbtn").click(function () {
            ...
            var url = SITEURL + "advertise5x/add/parentkey/<?php echo $parentkey;?>/itemid/<?php echo $itemid;?>/ismobile/"+ismobile;
        ...
        window.product_store = Ext.create('Ext.data.Store', {
            ...
            proxy: {
                type: 'ajax',
                api: {
                    read: SITEURL + 'advertise5x/index/action/read/ismobile/<?php echo $ismobile;?>',  //读取数据的URL
            ...
            columns: [
                ...
                },
                {
                    ...
                    renderer: function (value, metadata, record) {
                        ...
                        var custom = record.get('custom_label');
                        var label = prefix + '_' + number;
                        if(custom != null){
                            label+= ' / '+custom;
                        }
                        return label;
                ...
                },
                {
                    ...
                    sortable: false,
                    renderer: function (value, metadata, record, rowIndex, colIndex) {
                        var src = "<?php echo $GLOBALS['cfg_public_url'];?>";
...
function modify(id) {
    var ismobile = "<?php echo $ismobile;?>";
    var url = SITEURL + 'advertise5x/edit/parentkey/<?php echo $parentkey;?>/itemid/<?php echo $itemid;?>/id/' + id+'/ismobile/'+ismobile;
