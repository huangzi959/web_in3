...
class Common
{
    ...
    * */
    ...
    static function send_email_message($orderInfo)
    {
        ...
        //获取模型拼音
        $table = Model_Model::pinyin_by_id($orderInfo['typeid'],false);
    ...
    static function send_sms_message($orderInfo)
    {
        ...
        {
            $content = self::sms_message_replace($rs['msg'], $orderInfo);
            $msg->sendMsg($orderInfo['linktel'], Common::C('cfg_webname'), $content);
        }
        //供应商(短信)
        if (Common::C('cfg_supplier_msg_open') == 1)
        {
            $content = Common::C('cfg_supplier_msg');
            $content = self::sms_message_replace($content, $orderInfo);
            $supplier = self::get_supplier($table, $orderInfo['productaid']);
            if (!empty($supplier))
            {
                $msg->sendMsg($supplier['telephone'], Common::C('cfg_webname'), $content);
            }
        }
        //网站管理员(短信)
     ...
     * @return mixed
     */
    static function sms_message_replace($content, $orderInfo)
    {
        $content = str_replace('{#LINKMAN#}', $orderInfo['linkman'], $content);
        $content = str_replace('{#LINKNAME#}', $orderInfo['linkman'], $content);
        $content = str_replace('{#PRODUCTNAME#}', $orderInfo['productname'], $content);
        $content = str_replace('{#PRICE#}', $orderInfo['price'], $content);
        $content = str_replace('{#PHONE#}', $orderInfo['linktel'], $content);
        ...
        $content = str_replace('{#EMAIL#}', $orderInfo['email'], $content);
