...
{request "pub/header/typeid/$typeid/isbookpage/1"}
      ...
      <div class="book_type">
        ...
        <div class="book_con_box">
            ...
            {st:spot action="suit" productid="$info['id']"}
              {loop $data $r}
               <li data-suitid="{$r['id']}" data-jftp="{$r['jifentprice']}" data-price="{$r['ourprice']}" data-storage="{$r['number']}">{$r['title']}（库存：{if $r['number'] > 0}{$r['number']}{else}0{/if}）</li>
              {/loop}
            {/st}
          </ul>
        </div>
  ...
  <script>
      ...
      function count_price()
      {
          ...
          //积分抵现
          if($('#usejifen').val()>0 && totalprice>0){
              money=-parseFloat($('#jifen-price').html());
          }
          $('.totalprice').html(totalprice+money);
      }
